...
<style>    
        ...
        #report th { text-align:center !important;background:url(header_bkg.png) repeat-x scroll center left; color:#fff; padding:7px 15px; text-align:left;border-left: none; border-right: none;
        			border-top: 2px solid #542f82; border-bottom:  2px solid #542f82; color: black;}
        #report td {text-align:center; background:#fff none repeat-x scroll center left; color:#000; padding:7px 15px; font-size: 12px; padding: 20px;}
...
<form id="_frmFormSearch" method="get" action="">
	...
	<div align="center" style="font-size: 50px; color: black;">
		Q&A 게시판
	</div><br><br>
	<div align="center">
		<table id="report" class="ba_qna" style="border-bottom: 1px solid #e1e1e1; border-top: 1px solid #e1e1e1;width: 85%;">
			<%-- <col width="40px">
			<col width="450px">
			<col width="100px">
			<col width="180px"> --%>
							            ...
							            <div style="text-align: left !important; padding: 30px;">
											...
											<%if((!qalist.get(i).getTitle().equals("관리자 답변입니다.")) && (mem.getAuth() == 2)){%>
												...
												if(!allref2.contains(qalist.get(i).getRef())){ %>					
									 				<a href="ansWrite.do?ref=<%=qalist.get(i).getRef()%>&g_code=<%=qalist.get(i).getG_code()%>&secret=<%=qalist.get(i).getSecret()%>">답변 <span style="margin-right: 9px"></span> </a>
							               		<%} %>
							               <%} %>
							            	...
							            	<%if(qalist.get(i).getTitle().equals("관리자 답변입니다.") && mem.getAuth() == 2){ %>
							            		<div align="right" style="margin-right: 15;"><a href="ansDelete.do?seq=<%=qalist.get(i).getSeq()%>">삭제</a></div>
							            	<%} %>   
						            </td>					           
						        </tr>
	...
	<%if(mem.getAuth() == 0){ %>
		<div align="right" style="width: 993px;"><a href="qaWrite.do">글쓰기</a></div>
	<%}else if(mem.getAuth() == 2){ %>
		<div align="right" style="width: 993px;"><a href="qaAdminWrite.do">공지사항등록</a></div>
	<%} %>	
</div>
